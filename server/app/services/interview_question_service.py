...
BANKS_DIR = os.path.join(os.path.dirname(__file__), '..', 'data', 'interview_questions')
...
class InterviewQuestionService:
    ...
    def select_questions(self, required_skills: List[str], preferred_skills: List[str], candidate_skills: List[str], count: int) -> List[Dict[str, Any]]:
        """Selects interview questions. Focuses primarily on textual questions based on skills."""
        selected = []
        selected_ids = set()
        ...
        def sample_from_skills(skill_list, target_count):
            ...
            matching = [q for q in all_questions if any(s.lower() in skill_list_lower for s in q.get('skills', [])) and q['id'] not in selected_ids]
        ...
        # 40% required, 30% preferred, 30% candidate/general
        sample_from_skills(required_skills, int(count * 0.4))
        sample_from_skills(preferred_skills, int(count * 0.7))
